...
WHERE  Lower(sequence_schema) NOT IN ('v_temp_schema') AND identity_table_name IS NULL AND ({sequences})
...
WHERE Lower(table_schema) NOT IN ('v_temp_schema') AND ({tables})
...
WHERE NOT is_system_view AND Lower(table_schema) NOT IN ('v_temp_schema') AND ({views})
ORDER BY Lower(table_schema), Lower(table_name);
...
WHERE 
	...
	Lower(schema_name) NOT IN ('v_txtindex', 'txtindex', 'v_temp_schema') AND 
...
WHERE 
    ...
    (object_type != 'SCHEMA' OR Lower(object_name) NOT IN ('v_catalog', 'v_internal', 'v_monitor', 'public', 'v_txtindex', 'v_temp_schema')) AND 
    ({grants})
...
'''
    ...
    void process() {
        ...
        if (jobArgs.script_path == null) {
            Logs.Severe('Required argument "script_path"')
            return
        }
