...
В этой статье я хочу поделиться своим опытом разработки ETL и ETL процессов для хранилищ данных Vertica
с помощью Getl.  
...
выполнения, статическая компиляция для ускорения выполнения ресурсоемких участков кода, поддержка разработки DSL расширений, 
...
1. Groovy, это можно сделать прямо на Хабре за 15 минут: [статья Groovy за 15 минут](http://habrahabr.ru/post/122127/).
1. Дюбой IDE, который поддерживает Java и Groovy и обеспечивает комфортную разработку и отладку кода, например 
...
* Getl уже содержит ссылки на все зависимые библиотеки, поэтому в проект будут подтянуты Groovy и весь остальной, 
требуемой для его работы софт.

Для того, чтобы Getl видел Ваши РСУБД и Vertica, рекомендуется в проекте сделать директорию jdbc и скопировать туда
jar файлы используемых JDBC драйверов. А в gradle указать использовать их при запуске и отладке приложения в IDE:
```groovy
dependencies {
    compile(group: 'net.sourceforge.getl', name: 'getl', version:'4.4.2')
...
Такой скрипт можно вызвать даже без компиляции с помощью Groovy, если он конечно у Вас установлен в ОС. Для этого
нужно скопировать по пути инсталляции Groovy в директорий _libs_ все jar файлы, нужные для работы Getl, включая JDBC
драйвера. Далее можно прямо в ОС вызвать Getl скрипт из корневой директории проекта:
```shell script
...
Groovy скриптов из jar файла в классе Getl реализован лаунчер, которому можно в параметре _runclass_ передать, 
какой скрипт требуется запустить:   
```shell script
java -cp myproj.jar getl.lang.Getl runclass=demo.GetlDemo
``` 
P.S. Не забудьте в class path также добавить пути к всем jar файлам, нужным для работы Getl, включая JDBC драйвера.
...
@Field тип имя_параметра = значение_по_умолчанию // например @Field Date beforeDate 
...
verticaConnection('соединение', true) {
    ...
    // Вызывать при подключениии соединения к БД
    whenConnecting {
...
```
Из вышепредставленной "рыбы" видно:
...
производительности.
1. В объектах источников поддерживаются свойства и методы, которые позволяют управлять ими с кода сценария. 
...
1. Для облегчения механизма проверок корректности работы внутри процесса, в Getl добавлена поддержка GroovyAssert методов,
...
РСУБД, остается только на Java или Groovy написать парсинг входных данных.
...
* Для таблицы Vertica указывается тоже имя и такие же поля, как у таблицы Oracle, для varchar полей длина умножается
...
* В _thread_ мы объявляем, кто код будет выполняться многопоточно, задаем список элементов для обработки в потоках в 
...
Стоит помнить про правила потокобезопасной работы в коде Java/Groovy, не обращаться к переменным снаружи и не работать 
...
* При обьявлении локальной переменной для таблицы Vertica, в качестве соединения для неё указывается соединения из репозитория.
...
temp для java.
1. После выгрузки с помощью COPY FROM LOCAL полученный файл будет загружен в Vertica.
...
``` 
* В данном примере все объекты регистрируются в репозитории, вместо использования локальных переменных. Это позволяет
оценить читабельность кода, когда например описание объектов мы вынесли в отдельный сценарий и вызываем его в начале работы
нашего ETL процесса, а не каждый раз описываем соединения и таблицы. Для того, чтобы каждый раз не задавать соединение
при создании таблиц, использован оператор _useVerticaConnection_. Каждая регистрируемая ниже него в коде таблица Vertica 
будет сразу получать указанное в нем соединение, если только для неё явно не задано свое соединение.
...
* _fileCopier_ отвечает за процесс копирования данных. При вызове передаются параметры источника и приёмника фс, задается
маска захвата файлов, маска конечного расположения скопированных файлов в директориях от рутового пути приёмника, количество
попыток при сбоев копирования и порядок копирования файлов.
...
файла будет вызван свой COPY. Дополнительно указываются пути, в какие директории при загрузки файлов писать ошибки на записи,
которые не прошли проверку Vertica на корректность. По окончании загрузки, файлы будут удалены, так как включена опция
_removeFile_.

В итоге, при вызове сценария, Getl просмотрит на ftp директории, которые по имени могут быть распарсены в дату,
скопирует все новые файлы оттуда на локальную фс, по другому сгруппировав файлы и одним запросом загрузит все файлы
в Vertica. Простота написания и легкая читабельность кода на лицо. Не смотря на простоту, _fileCopier_ поддерживает 
множество режимов работы, в том числе зеркальное копирование на несколько источников одновременно или сегментное
копирование на несколько источников по заданному хэш ключу. Последнее позволяет из одного источника взять файлы, раскидать
их равномерно веером по кластеру ETL серверов и на каждом из них запустить свою обработку доставленных данных. Вполне себе
простой аналог MPP архитектуры.      
...
источника не указана таблица истории, если в ней появяться файлы с такими же именами, они снова будут загружены.
...
поля (например поле находится в группа.аттрибут), то точный способ получения нужно указать в _alias_. Так как поле
...
так как файлы будут обрабатываться многопоточно и гарантируется только, что на момент запуска парсера список для обработки
...
* При процессинге json файлу установливается, по какому пути находится файл для обработки и как он называется. Как и говорилось
...
* В _copyRows_ копируются совпадающие по именам поля из json файла в мастер таблицу Vertica. Дополнительно в _childs_
...
если details не пустой. В Java нам пришлось бы писать условие "if (source_row.details != null)". 
...
* В зависимости от того, какое результат указан в _result_, Getl будет производить разный набор действий:
    * Для completeResult файл будет считаться успешно обработанным. Он будет перенесен в файловый сторайдж архивных файлов, 
    если он задан в _storageProcessedFiles_. Если включено _removeFiles_, то файл будет удален с источника.
    * для errorResult файл будет считаться ошибочным. Он будет перенесен в файловый сторайдж ошибочных файлов,
    если он задан в _storageErrorFiles_. Вместе с файлом рядом будет создан текстовый файл с описанием ошибки и 
...
запуская код обработки файлов в 16 потоков. Для каждого найденого файла в Vertica будет создано отдельное соединение,
...
* Getl запоминает, какие внешние подключения были связаны с соединением Vertica. Не удасться второй раз присоединить тот же
...
записей, а так же пересчитывать статистику. Getl позволяет упростить этот процесс и написать свой небольшой сценарий:
```groovy
verticaConnection {
    connectHost = 'vertica-host1'; connectDatabase = 'verdb'
    extended.backupservernode = 'vertica-host2,vertica-host3'
...
``` 
* _purgeTables_ сделает дефраментацию всех таблиц БД Vertica, у которых процент удаленных записей от общего количества 
превышает 15%.
...
Напишем ETL процесс работы с сценарием подключения
```groovy
package data
...
```
* При загрузке конфигураций Getl их объединяет в памяти. Если параметры нового файла перекрывают уже существующие, то 
они будут записаны поверх текущих.
...
# Заключение о Getl
Сложно описать в одной статье восьми летний опыт разработки ХД проектов и поделиться функциональными возможностями
заточенного под это продукта. Несмотря на приличный объем, я описал только небольшую часть того, что умеет Getl. Надеюсь
я смог дать базовые понятия, для чего Getl нужен, как им пользоваться и в какую сторону копать при его изучении.

Примеры работы с его объектами и dsl языком можно найти в юнит тестах самого проекта Getl. Небольшой пример проекта ХД на
