...
файловых источников с сложными форматами и таблиц РСУБД. В Getl не нужно разрабатывать для каждой таблицы или формата файла
...
которой выложены на GutHub проекте [getl.vertica](https://github.com/ascrus/getl.vertica)
...
запускаемых задач из юнит тестов;
...
Для того, чтобы начать писать на Getl процессы ETL, сначала нужно на базом уровне изучить два продукта:
...
```groovy
dependencies {
    compile(group: 'net.sourceforge.getl', name: 'getl', version:'4.5.0')
}
```
...
Для того, чтобы Getl видел Ваши РСУБД, рекомендуется в проекте сделать директорию jdbc и скопировать туда
...
Getl рекомендуется описать нужные действия в методе с анотацией @Before.
