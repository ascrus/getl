...
Getl является заменой классических ETL для проектов для случаев, когда требуется быстрая разработка процессов загрузки данных
...
или формата файла свой процесс. Достаточно на простом и мощном ETL языке создать шаблоны с нужной логикой работы с данными 
и источниками под логику программной архитектуры проекта и вызывать их для нужных источников. Getl является
...
* Это руководство по разработке на Getl располагается в файле 
[getl.rus.md](https://github.com/ascrus/getl/blob/master/manuals/getl.rus.md).
...
# Примеры работы
* Примеры работы с его классами и Dsl языком можно найти в 
...
# Развитие
Getl является перспективным ETL инструментом компании EasyData. Он позволяет быстро и качественно решать задачи любой сложности 
...
Для подключения к проекту на "_gradle_" достаточно прописать в файл "_build.gradle_" проекта ссылку на Getl:
```groovy
dependencies {
...
* операторы callScript и callScripts: однократный или многократный вызов другого скрипта
...
profile('Копирование из A в Б') {
    ...
    callScript new пакет.Проверка(параметр1: verticaTable('vertica:table'))
...
Getl поддерживает работу со следующими источниками данных:
...
## Чтение и запись в источниках данных
В Getl поддерживается чтение и запись данных в JDBC источники и файлы. Для разных источников есть ограничения:
* Не поддерживается вставка записей в Hive.
* Не поддерживается изменение и удаление записей в Hive и Impala.
* Пакетная загрузка файлов поддерживается только для РСУБД H2, Hive, Impala и Vertica.
* Не поддерживается запись данных в файлы формата JSON, XML, YAML.
* Не поддерживается вставка и изменение записей в NetSuite и SalesForce.

### Операторы записи данных в таблицы и файлы
...
### Операторы чтения данных из таблиц и файлов
Для чтения записей у таблиц есть функции _rows_ и _eachRow_. Первая функция возвращает все записи в виде списка. Вторая функция перебирает по очереди каждую запись и выполняет указанный
код над ней.

Пример записи данных по условию "id меньше 1000" из заполненной данными в прошлом примере таблицы Vertica в JSON и CSV файлы:
```groovy
verticaTable { vertab ->
    useVerticaConnection vercon
    tableName = 'random_data'
    ...
    // Описание CSV файла
    def csv = csv {
        // Определение пути и имени файла (в таком случае не требуется указывать _connection_)
        fileName = '/data/files/file1.txt'
        // Определение формата записи CSV
        fieldDelimiter = ';'
        codePage = 'utf-8' 
    }
    ...
    etl.rowsTo(csv) {
        writeRow { writer ->
            // Перебрать записи таблицы Vertica по заданным условиям и сортировке
            vertab.eachRow(where: 'id < 1000', order: ['id']) { row ->
                // Записать поля записи в CSV файл
                writer row
            }
        }
    }

    // Создать писатель в JSON файл
    def writer = new File('/data/files/file1.json').newWriter('utf-8')
    // Определить построитель JSON данных
    def builder = new StreamingJsonBuilder(writer)
    // Построить JSON из записей таблицы Vertica по заданным условиям и сортировке
...
* Команда _ECHO_ для выдачи текста в консоль лога.
...
# Расширения в Getl под Micro Focus Vertica
...
/* ... сам код перерасчета на промежуточной таблице ... */
...
verticaConnection('dest') {
    // Подсоединение соединения источника к текущему соединению
...
Пример создания схему хранения и таблиц эталонных данных:
```groovy
models.referenceVerticaTables('model2') { createReferenceTables() }
```
P.S. Если таблицы уже присутствуют в БД, то они будут пропущены и не пересозданы. Для пересоздания таблиц при вызове метода 
